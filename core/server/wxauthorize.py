...
class WxAuthorServer(object):
    ...
    def get_auth_access_token(self, code):
        ...
        r = requests.get(url)
        logger.debug(
            '【微信网页授权】通过code换取网页授权access_token的Response[' + str(r.status_code) + ']')
    ...
    def get_userinfo(self, access_token, openid):
        ...
        url = self.get_userinfo_url + \
            'access_token=%s&openid=%s&lang=zh_CN' % (access_token, openid)
        r = requests.get(url)
...
check_signature:校验signature是否正确
    """
    sys_order_reply = '您好，欢迎您关注涛声明月！'
    # def __init__(self):
    #     self.sys_order_reply = '您好，欢迎您关注涛声明月！'
    ...
    def reply_text(self, FromUserName, ToUserName, CreateTime, Content):
        ...
        # <MsgType><![CDATA[%s]]></MsgType> <Content>
        # <![CDATA[%s]]></Content></xml>"""
        textTpl = """<xml>
        ...
        out = textTpl % (FromUserName, ToUserName,
